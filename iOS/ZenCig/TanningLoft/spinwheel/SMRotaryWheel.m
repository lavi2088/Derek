...
- (void) spin {
    float x = arc4random_uniform(10);
    CABasicAnimation *rotation;
    rotation = [CABasicAnimation animationWithKeyPath:@"transform.rotation"];
    rotation.fromValue = [NSNumber numberWithFloat:0];
    rotation.toValue = [NSNumber numberWithFloat:(2*M_PI)];
    rotation.duration = 0.5; // Speed
    rotation.repeatCount = x; // Repeat spin twice.
    [container.layer addAnimation:rotation forKey:@"Spin"];
    
    [self performSelector:@selector(endAnimation) withObject:nil afterDelay:0.5*x];
   // [self spinBall];
}
